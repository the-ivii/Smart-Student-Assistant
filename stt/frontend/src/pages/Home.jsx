...
export default function Home() {
  ...
  const handleSubmit = async (topic, mode) => {
    ...
    // Create new abort controller for this request (before try block so it's accessible in catch/finally)
    ...
    try {
      ...
      }, 60000) // Increased to 60 seconds for deployed backend
      ...
      // Don't show error if request was intentionally aborted (e.g., new request started)
      // Check if the abort controller was replaced (meaning a new request started)
      if (err.name === 'AbortError') {
        // If the controller was replaced, this was an intentional cancellation
        if (abortControllerRef.current !== currentController) {
          // New request started, ignore this error
          return
        }
        // Otherwise it's a timeout - show error
      ...
      setLoading(false)
      // Only clean up if this is still the current request
      if (abortControllerRef.current === currentController) {
